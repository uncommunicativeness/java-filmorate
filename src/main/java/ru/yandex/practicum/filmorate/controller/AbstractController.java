...
public abstract class AbstractController<T extends AbstractData> {
    ...
    public ResponseEntity<ValidationErrorResponse> handleException(MethodArgumentNotValidException e) {
        return new ResponseEntity<>(new ValidationErrorResponse(e.getBindingResult().getFieldErrors().stream().map(error -> new ValidationErrorResponse.ValidationError(error.getField(), error.getDefaultMessage())).peek(error -> log.warn(error.getMessage())).collect(Collectors.toList())), HttpStatus.BAD_REQUEST);
    }
